...
if __name__ != "__main__":
    ...
    patternNames = {
        name: obj
        for name, obj in vars(Sequences).items()
...
re_input = re_input.format(
    player=re_player, synthdef=re_synthdef, args=re_args, methods=re_methods
)
...
def createPlayer(**kwargs):
    ...
    sentence = (
        "{player} >> {synthdef}({degree}"
        + (", " if (len(degree) > 0 and len(keywords) > 0) else "")
        + "{keywords}){methods}"
    )
...
def format_kwargs(d):
    return ", ".join(
        ["{}={}".format(*item) for item in sorted(d.items(), key=lambda x: str(x[0]))]
    )
...
def GENERATE_LIST(keyword=None):
    return "[{}]".format(
        ", ".join([str(GENERATE_NUMBER(keyword)) for n in range(randint(2, 9))])
    )


def GENERATE_TUPLE(keyword=None):
    return "({})".format(
        ", ".join([str(GENERATE_NUMBER(keyword)) for n in range(randint(2, 3))])
    )
...
ActionWeights = {ADD_NEW_KWARG: 2, CHANGE_DEGREE: 5, CHANGE_KWARG: 3, ADD_NEW_METHOD: 0}
