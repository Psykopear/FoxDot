...
class __scale__:
    ...
    justMajor = ScalePattern(
        [0, 2, 4, 5, 7, 9, 11], name="justMajor", tuning=Tuning.just
    )
    justMinor = ScalePattern(
        [0, 2, 3, 5, 7, 8, 10], name="justMinor", tuning=Tuning.just
    )
    ...
    bebopDorian = ScalePattern(
        [0, 2, 3, 4, 5, 9, 10], name="bebopMin"
    )  # aka Bebop Minor
    bebopDom = ScalePattern(
        [0, 2, 4, 5, 7, 9, 10, 11], name="bebopDom"
    )  # Bebop Dominant/Mixolydian
    bebopMelMin = ScalePattern(
        [0, 2, 3, 5, 7, 8, 9, 11], name="bebopMelMin"
    )  # Bebop Melodic Minor
        ...
        for items in (self.__class__.__dict__.items(), self.__dict__.items()):
            lib.extend(
                [
                    ...
                    if isinstance(value, ScalePattern)
                ]
            )
        return dict(lib)
