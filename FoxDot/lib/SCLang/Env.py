...
class EnvGen(instance):
    ...
    def __str__(self):
        return str(
            cls("EnvGen").ar(instance(self.__attr__()), doneAction=self.doneAction)
        )

    def __attr__(self):
        """ Converts the attr dict to SCLang arguments """
        args = ",".join(
            ["{}: {}".format(str(key), str(value)) for key, value in self.attr.items()]
        )
...
class mask(EnvGen):
    def __init__(self, sus=None, amp=None, curve="'lin'", doneAction=0):
        self.attr = {}
        self.attr["times"] = (
            [0.01, instance("sus") - 0.01, 0.01] if sus is None else sus
        )
...
class reverse(EnvGen):
    def __init__(self, sus=None, amp=None, curve="'exp'", doneAction=0):
        ...
        self.attr["levels"] = (
            [0.0001] + ([instance("amp")] if amp is None else amp) + [0]
        )
        self.attr["curve"] = curve
        self.doneAction = doneAction
