"""
NOTE: File directly copied from https://raw.githubusercontent.com/MTG/freesound-python/master/freesound.py
...
class Sound(FreesoundObject):
    ...
    def retrieve(self, directory, name=False, reporthook=None):
        ...
        For further reference, check the urllib docs.
        """
        self.path = os.path.join(directory, name if name else self.name)
        uri = URIS.uri(URIS.DOWNLOAD, self.id)
        return FSRequest.retrieve(uri, self.client, self.path, reporthook)
