...
class PGroupOr(metaPGroupPrime):
    ...
    def calculate_time(self, *args, **kwargs):
        ...
        samp_delay = (
            self.meta[0].calculate_time(*args, **kwargs)
            if isinstance(self.meta[0], PGroup)
            else 0
        )
...
def bubble(self, size=2):
    ...
    for n in range(len(self.data)):
        if n % 4 == 0:
            new.append(
                [self.data[n], PGroupPlus(self.data[n], modi(self.data, n + size))]
            )
        elif n % 4 == 2:
            ...
            new.append(
                [
                    PGroupPlus(self.data[n], self.data[n - 1]),
