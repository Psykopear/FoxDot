...
with SampleSynthDef("play1") as play:
    play.osc = PlayBuf.ar(
        1,
        play.buf,
        BufRateScale.ir(play.buf) * play.rate,
        startPos=BufSampleRate.kr(play.buf) * play.pos,
    )
...
with SynthDef("lazer") as synth:
    ...
    synth.osc = VarSaw.ar(synth.freq, width=(synth.rate - 1) / 4) + LFSaw.ar(
        LFNoise0.ar(
            synth.rate * 20,
            add=synth.freq * Pulse.ar((synth.rate - 2) + 0.1, add=1),
            mul=0.5,
        )
    )
...
with SynthDef("growl") as growl:
    ...
    growl.osc = SinOsc.ar(
        growl.freq + SinOsc.kr(0.5, add=1, mul=2), mul=growl.amp
    ) * Saw.ar((growl.sus / 1.5) * 32)
    growl.env = Env.env()
...
with SynthDef("bass") as bass:
    ...
    bass.osc = (
        LFTri.ar(bass.freq, mul=bass.amp)
        + VarSaw.ar(bass.freq, width=bass.rate / 10, mul=bass.amp)
        + SinOscFB.ar(bass.freq, mul=bass.amp / 2)
    )
    bass.env = Env.perc(atk=0.02, curve="'lin'")
...
with SynthDef("dirt") as dirt:
    ...
    dirt.amp = dirt.amp / 2
    dirt.osc = (
        LFSaw.ar(dirt.freq, mul=dirt.amp)
        + VarSaw.ar(dirt.freq + 1, width=0.85, mul=dirt.amp)
        + SinOscFB.ar(dirt.freq - 1, mul=dirt.amp / 2)
    )
    dirt.env = Env.perc()
...
with SynthDef("rave") as rave:
    rave.osc = Gendy1.ar(
        rave.rate - 1, mul=rave.amp / 2, minfreq=rave.freq, maxfreq=rave.freq * 2
    )
    rave.env = Env.perc()

with SynthDef("scatter") as scatter:
    scatter.osc = (
        Saw.ar(scatter.freq, mul=scatter.amp / 8)
        + VarSaw.ar(scatter.freq + [2, 1], mul=scatter.amp / 8)
    ) * LFNoise0.ar(scatter.rate)
...
with SynthDef("charm") as charm:
    ...
    charm.osc = SinOsc.ar(charm.freq, mul=charm.amp / 4) + VarSaw.ar(
        charm.freq * 8, 10, mul=charm.amp / 8
    )
    charm.osc = LPF.ar(
        charm.osc,
        SinOsc.ar(
            Line.ar(1, charm.rate * 4, charm.sus / 8),
            0,
            charm.freq * 2,
            charm.freq * 2 + 10,
        ),
    ...
    bell.osc = (
        Klank.ar(
            [
                ...
                stutter([1.2, 0.9, 0.25, 0.14, 0.07], 3),
            ],
            Impulse.ar(0.01),
            bell.freq,
            0,
...
with SynthDef("soprano") as soprano:
    ...
    soprano.osc = SinOsc.ar(soprano.freq * 3, mul=soprano.amp) + SinOscFB.ar(
        soprano.freq * 3, mul=soprano.amp / 2
    )
    soprano.env = Env.env()
...
with SynthDef("viola") as viola:
    ...
    viola.osc = PMOsc.ar(
        viola.freq,
        Vibrato.kr(viola.freq, rate=viola.vib, depth=0.008, delay=viola.sus * 0.25),
        10,
        mul=viola.amp / 2,
    )
...
with SynthDef("scratch") as scratch:
    ...
    scratch.osc = SinOsc.ar(
        Vibrato.kr(
            scratch.freq, 2, 3, rateVariation=scratch.rate, depthVariation=scratch.depth
        ),
        mul=scratch.amp,
    )
    scratch.env = Env.env()
...
with SynthDef("klank") as klank:
    ...
    klank.osc = Klank.ar(
        [[1, 2, 3, 4], [1, 1, 1, 1], [2, 2, 2, 2]], ClipNoise.ar(0.0005).dup, klank.freq
    )
    if SC3_PLUGINS:
...
with SynthDef("feel") as feel:
    ...
    feel.osc = Klank.ar(
        [[1, 2, 3, 3 + ((feel.rate - 1) / 10)], [1, 1, 1, 1], [2, 2, 2, 2]],
        Impulse.ar(0.0005) * Saw.ar(feel.freq, add=1),
        feel.freq,
    )
...
with SynthDef("glass") as glass:
    ...
    glass.osc = Klank.ar(
        [[2, 4, 9, 16], [1, 1, 1, 1], [2, 2, 2, 2]],
        PinkNoise.ar(0.0005).dup * SinOsc.ar(glass.freq / 4, add=1, mul=0.5),
        glass.freq,
    )
...
with SynthDef("soft") as soft:
    ...
    soft.osc = Klank.ar(
        [[7, 5, 3, 1], [8, 4, 2, 1], [2, 4, 8, 16]],
        LFNoise0.ar(soft.rate / soft.sus),
        soft.freq,
    )
...
with SynthDef("quin") as synth:
    ...
    synth.osc = (
        Klank.ar(
            [[1, 2, 4, 2], [100, 50, 0, 10], [1, 5, 0, 1]],
            Impulse.ar(synth.freq).dup,
            synth.freq,
        )
        / 5000
    )
...
with SynthDef("pluck") as pluck:
    ...
    pluck.osc = (
        SinOsc.ar(freq * 1.002, phase=VarSaw.ar(freq, width=Line.ar(1, 0.2, 2))) * 0.3
        + SinOsc.ar(freq, phase=VarSaw.ar(freq, width=Line.ar(1, 0.2, 2))) * 0.3
    )
    pluck.osc = (
        pluck.osc
        * XLine.kr(pluck.amp, pluck.amp / 10000, pluck.sus * 4, doneAction=2)
        * 0.3
    )
...
with SynthDef("spark") as synth:
    ...
    synth.osc = (
        LFSaw.ar(freq * 1.002, iphase=Saw.ar(0.1)) * 0.3
        + LFSaw.ar(freq, iphase=Saw.ar(0.1)) * 0.3
    )
    synth.osc = (
        synth.osc * Line.ar(synth.amp, synth.amp / 10000, synth.sus * 1.5) * 0.3
    ) * Line.ar(0.01, 1, synth.sus * 0.033)
spark = synth
...
with SynthDef("blip") as synth:
    ...
    synth.osc = (
        LFCub.ar(freq * 1.002, iphase=1.5)
        + LFTri.ar(freq, iphase=Line.ar(2, 0, 0, 2)) * 0.3
    ) * Blip.ar(freq / 2, synth.rate)
...
with SynthDef("ripple") as ripple:
    ...
    ripple.osc = Pulse.ar(ripple.freq / 4, 0.2, 0.25) + Pulse.ar(
        ripple.freq + 1, 0.5, 0.5
    )
...
orient.osc = LFPulse.ar(orient.freq, 0.5, 0.25, 1 / 4) + LFPulse.ar(
    orient.freq, 1, 0.1, 1 / 4
)
orient.env = Env.perc()
orient.add()
...
zap.amp = zap.amp / 10
zap.osc = Saw.ar(zap.freq * [1, 1.01] + LFNoise2.ar(50).range(-2, 2)) + VarSaw.ar(
    zap.freq + LFNoise2.ar(50).range(-2, 2), 1
)
zap.env = Env.perc(atk=0.025, curve=-10)
zap.add()

marimba = SynthDef("marimba")
marimba.osc = Klank.ar(
    [[1 / 2, 1, 4, 9], [1 / 2, 1, 1, 1], [1, 1, 1, 1]],
...
twang.osc = Env.perc() * CombL.ar(
    twang.osc, delaytime=twang.rate / (twang.freq * 8), maxdelaytime=0.25
)
twang.add()
...
nylon.osc = LFPulse.ar(nylon.freq, 0.5, 0.33 * nylon.rate, 0.25) + LFPar.ar(
    nylon.freq + 0.5, 1, 0.1, 0.25
)
...
donk.osc = Ringz.ar(
    Impulse.ar(0, phase=donk.rate) / (donk.rate + 1), donk.freq, donk.sus, donk.amp
)
donk.add()
...
swell.osc = VarSaw.ar(
    swell.freq,
    width=SinOsc.ar(swell.rate / (2 * swell.sus / 1.25), add=0.5, mul=[0.5, 0.5]),
    mul=[1, 0.5],
)
swell.env = Env.perc()
swell.add()
...
razz.osc = Saw.ar(razz.rate * [1, 1 / 2], [1, 1 / 3]) + Saw.ar(
    razz.rate + LFNoise2.ar(4).range(0.5, 2.5), 1
)
...
with SynthDef("star") as synth:
    ...
    synth.freq = synth.freq / 2
    synth.osc = (
        LFSaw.ar(freq * 1.002, iphase=VarSaw.kr(freq, width=Line.kr(1, 0.2, synth.sus)))
        * 0.3
        + LFSaw.ar(
            freq + LFNoise2.ar(50).range(-2, 2) + 2,
            iphase=VarSaw.kr(freq + 2, width=Line.kr(1, 0.2, synth.sus)),
        )
        * 0.3
    )
    synth.osc = (
        synth.osc
        * XLine.ar(synth.amp, synth.amp / 10000, synth.sus * 3, doneAction=2)
        * Line.ar(0.01, 0.5, 0.07)
    )
star = synth
