        ...
        for i in range(
            LCM(
                *[
                    len(arg)
                    for arg in args
                    if (hasattr(arg, "__len__") and not isinstance(arg, PGroup))
                ]
            )
        ):
    ...
    def new_function(self, *args):
        ...
        for i in range(
            ...
            pat |= f(
                self,
                *[
                    (modi(arg, i) if not isinstance(arg, PGroup) else arg)
                    for arg in args
                ]
            )
        return pat
...
class metaPattern(object):
    ...
    def transform(self, func):
        return self.__class__(
            [
                (item.transform(func) if isinstance(item, metaPattern) else func(item))
                for item in self
            ]
        )
    ...
    def __str__(self):
        try:
            ...
            val = self.data
        return (
            "P" + self.bracket_style[:-1] + (repr(val)[1:-1]) + self.bracket_style[-1]
        )
    ...
    def convert_data(self, dtype=float, *args, **kwargs):
        ...
        new = map(
            (
                lambda x: x.convert_data(dtype, *args, **kwargs)
                if isinstance(x, metaPattern)
                else dtype(x, *args, **kwargs)
            ),
            self.data,
        )
    ...
    def getitem(self, key, get_generator=False):
        ...
        # We can get items using a slice
            ...
            i = key % len(self.data)
            val = self.data[i]
            if isinstance(val, (Pattern, Pattern.Pvar)) or (
                isinstance(val, GeneratorPattern) and not get_generator
            ):
                j = key // len(self.data)
        ...
        if isinstance(key, slice):
            ...
            if isinstance(self.data[i], metaPattern):
                ...
                if key >= len(self.data):
                    self.data[i] = Pattern([self.data[i], Format(value)]).stutter(
                        [key // len(self.data), 1]
                    )
                else:
    ...
    def deep_shuffle(self, n=1):
        ...
        for i in range(n):
            data = [
                (item if not isinstance(item, metaPattern) else item)
                for item in self.data[:]
            ]
            shuffle(data)
            items.extend(data)
    ...
    def map(self, func):
        ...
        return self.new(
            [
                (item.map(func) if isinstance(item, metaPattern) else func(item))
                for item in self.data
            ]
        )
        ...
        if len(l) > 0:
            ...
            for key in range(LCM(*l)):
                new_data.append(
                    self.__class__(
                        [
                            item.getitem(key) if isinstance(item, Pattern) else item
                            for item in self.data
                        ]
                    )
                )
...
def force_pattern_args(f):
    ...
    def new_func(*args, **kwargs):
        ...
        new_kwargs = {
            key: value if isinstance(value, metaPattern) else value
