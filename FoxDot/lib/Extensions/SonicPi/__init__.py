...
class SonicPiSynthDef(CompiledSynthDef):
    ...
    def preprocess_osc(self, osc_msg):
        ...
        if (
            "attack" not in osc_msg
            and "decay" not in osc_msg
            and "sustain" not in osc_msg
            and "release" not in osc_msg
        ):
...
def LoadSonicPiSynthsDev(sonic_pi_dir):
    ...
    synthinfo = os.path.join(
        sonic_pi_dir, "app/server/sonicpi/lib/sonicpi/synths/synthinfo"
    )
    ...
    _load_synths(
        ...
        lambda fn: os.path.join(
            sonic_pi_dir, "etc/synthdefs/compiled/", fn + ".scsyndef"
        ),
    )
...
def LoadSonicPiSynths(metadata_file=SONIC_PI_FILE):
    ...
    def create_filename(fullname):
        ...
        if not os.path.exists(filename):
            url = (
                "https://github.com/samaaron/sonic-pi/raw/%s/etc/synthdefs/compiled/%s.scsyndef"
                % (ref, fullname)
            )
